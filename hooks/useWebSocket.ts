  ...
  const initializeConnections = useCallback(async () => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        // Check if internal WebSocket is enabled
        const enableInternalWs = process.env.NEXT_PUBLIC_ENABLE_INTERNAL_WS !== 'false'
        ...
        if (enableInternalWs) {
          ...
          console.log('ℹ️ Internal WebSocket disabled via NEXT_PUBLIC_ENABLE_INTERNAL_WS=false')
          if (isMountedRef.current) {
            setIsInitialized(true)
          }
        }
